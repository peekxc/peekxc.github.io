...
$A : \mathbb{R}^n \to \mathbb{R}^n$ can be _diagonalized_ in terms of
...
_orthogonal diagonalization_. Such matrices are among the most commonly
...
## Lanczos on a bumper sticker {.scrollspy}

Given any non-zero $v \in \mathbb{R}^n$, Lanczos generates a _Krylov
subspace_ via successive powers of $A$:
...
Since $A$ is symmetric, $T$ is guaranteed to have a _symmetric
tridiagonal structure_:
...
Parlett](https://apps.dtic.mil/sti/tr/pdf/ADA289614.pdf), _could
anything be more simple?_

## The “iteration” part {.scrollspy}

Lanczos originally referred to his algorithm as the _method of minimized
iterations_, and indeed nowadays it is often considered an [iterative
...
theory, this means we can _iteratively_ generate both $Q$ and $T$ using
just a couple vectors at a time—no explicit QR call needed. Pretty
nifty, eh!

## Wait, isn’t $T$ arbitrary? {.scrollspy}
...
infinitely many _essentially distinct_ such decompositions.
...
…this _is_ a **QR** factorization, which is [essentially
...
$Q^T A Q = T$, then $Q$ and $T$ are _uniquely determined_ by $(A, q_1)$.

<!-- Moreover, so long as $T$ has non-zero subdiagonal entries, its the eigenvalues must be distinct -->

## The end of the beginning {.scrollspy}
...
examples including the _Rayleigh Ritz_ procedure, the _conjugate
gradient_ method, and notions of _Gaussian quadrature_. As each of these
...
_Proceedings of the Twenty-Ninth Annual ACM-SIAM Symposium on Discrete
Algorithms_, 1605–24. SIAM.
...
Lanczos Algorithm Yet.” _Brown Et Al_ 3: 93–107.
...
Systems with Several Right-Hand Sides.” _Mathematics of Computation_ 48
(178): 651–62.
