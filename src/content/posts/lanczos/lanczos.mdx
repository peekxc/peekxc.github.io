...
In other words, if ($q_{j\text{-}1}, \beta_j, q_j$) are known, then ($\alpha_j$, $\beta_{j+1}, q_{j+1}$) are completely determined. In theory, this means we can *iteratively* generate both $Q$ and $T$ using just a couple vectors at a time—no explicit QR call needed. 

To some, this may seem like just natural consequence of the recurrence relation...

## ... but isn't $T$ arbitrary?
...
However, it turns out that $T$ is actually fully characterized by the pair $(A, v)$. To see this, notice that since $Q$ is an orthogonal matrix, we have:
